...
<IfModule mod_ssl.c>
    <VirtualHost *:443>
        ...
        # Оптимизация производительности - Gzip сжатие
...
if [ "$APP_ENV" = "prod" ] && [ ! -z "$ALLOWED_DOMAIN" ]; then
    ...
    # Порядок важен: сначала защита от ботов, потом редиректы, потом ограничение по домену
    {
        ...
        echo "    # Если файл существует, отдаем его напрямую (приоритет 1 - проверяем первым для скорости)"
        echo "    RewriteCond %{REQUEST_FILENAME} -f"
        echo "    RewriteRule ^(img|css|js|files)/(.*)$ - [L]"
        echo ""
        echo "    # ============================================"
        echo "    # Защита от ботов и rate limiting (только для публичных страниц)"
        echo "    # ============================================"
        echo "    # Исключаем админ-панель из проверок ботов для производительности импорта"
        echo "    # НЕ используем [L] здесь, чтобы запрос продолжал обрабатываться CakePHP"
        echo ""
        echo "    # Блокировка известных ботов и краулеров (кроме основных поисковых)"
        echo "    # Разрешаем только Googlebot и Yandex для SEO"
        echo "    # Пропускаем проверку для админ-панели"
        echo "    RewriteCond %{REQUEST_URI} !^/admin [NC]"
        echo "    RewriteCond %{REQUEST_URI} !^/app/webroot [NC]"
