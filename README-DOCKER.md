...
## Настройка SSL сертификата (Production)

### Автоматическая настройка Let's Encrypt

Для первоначальной настройки SSL сертификата от Let's Encrypt:

```bash
# Убедитесь, что домен указывает на ваш сервер (A запись -> IP)
...
- Получит сертификат
- Скопирует в Docker volume
- Настроит автоматическое обновление

### Обновление SSL сертификата

Для ручного обновления сертификата:

```bash
./update-ssl-cert.sh example.com
```

Сертификат обновляется автоматически через cron (каждый день в 3:00).
