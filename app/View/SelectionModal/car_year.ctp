...
<div class="popup__content">
    <div class="selection__modal">
            ...
            <div class="selection__modal__wrapper">
                ...
                <div class="selection__modal-list">
                    <?php foreach ($item['list'] as $i => $listItem) { ?>
                        <?php if ($i > 0 && $i % 4 == 0) { ?>
                        <?php } ?>
                        <a class="selection__modal-list-item" href="javascript:void(0);" onclick="setYearId(<?php echo $listItem['name']; ?>);"><?php
                            echo $listItem['name']; ?>
                        </a>
                    <?php } ?>
                </div>
        ...
        <script type="text/javascript">
            function setYearId(valueToSelect) {
                $("#CarYear").val(valueToSelect).change();
                close_popup();
                open_popup({
                    url: `/selection-modal/<?php echo $brand_id;?>/<?php echo $model_id;?>/${valueToSelect}`,
                    type: 'post',
                    size: 'lg'
                });
            }
