<div class="left-nav left-nav-open" id="left-nav-filter">
    <div class="left-nav__sticky">
        ...
        <!-- Индикатор загрузки для кнопок поиска -->
        <script type="text/javascript">
        $(document).ready(function() {
            // Функция для создания мигающих точек
            function createLoadingDots() {
                var dots = '';
                for (var i = 0; i < 3; i++) {
                    dots += '<span class="loading-dot" style="animation-delay: ' + (i * 0.2) + 's;">.</span>';
                }
                return dots;
            }
            
            // Добавляем CSS для анимации точек один раз
            if (!$('#loading-dots-style').length) {
                $('head').append('<style id="loading-dots-style">' +
                    '.loading-dot {' +
            ...
            // Обработчик для всех кнопок поиска в filter-group
            $(document).on('click', '.filter-group button.bt-style1', function(e) {
                var $button = $(this);
                var originalText = $button.html().trim();
                
                // Пропускаем, если кнопка уже в состоянии загрузки
                ...
                // Устанавливаем текст с точками
                $button.html('ПОИСК' + createLoadingDots());
                
                // Для кнопок с type="button" и onclick (например, onSearchModifications)
                // не блокируем выполнение их функции
                if ($button.attr('type') === 'button' && $button.attr('onclick')) {
                    // Функция выполнится сама, кнопка уже показывает загрузку
                    return true;
                }
                
                // Для остальных кнопок форма отправится автоматически (type="submit" по умолчанию)
                // или через обработчик формы
            });
            
            // Также обрабатываем отправку формы через submit
            $(document).on('submit', '.filter-group form', function(e) {
                var $form = $(this);
                var $button = $form.find('button.bt-style1');
            ...
            // Восстанавливаем кнопку при возврате на страницу (если была ошибка)
            $(window).on('pageshow', function(event) {
                if (event.originalEvent && event.originalEvent.persisted) {
                    $('.filter-group button.bt-style1').each(function() {
                        var $button = $(this);
