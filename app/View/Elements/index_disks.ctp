				...
				<?php if ($mode == 'table' || count($item['Product']) == 1 ) { ?>
					...
					<?php if ($mode == 'block') { ?>
						...
						<div class="priceMore disks">
                            ...
                            echo $this->element('stock_places', array('stock_places' => $item['Product'][0], 'text' => '<div class="number disks">'.$this->Frontend->getStockCount($item['Product'][0]['stock_count']).' шт. '.$in_stock_mark.'</div>', 'position' => 'center')); ?>
