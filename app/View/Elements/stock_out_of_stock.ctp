...
<?php if (!empty($item[$count_name])) { ?>
    ...
    if (!empty($supplier['prefix'])) {
        $prefix_supplier = ' (' . $supplier['prefix'] . ') ';
    }
    ?>

    <div class="tooltip-places tooltip-places-center tooltip-out-of-stock">
        <span class="<?php if (!empty($hide_prefix_on_mobile)) {
            echo 'tooltip-places__prefix';
        } ?>"><?php echo $prefix . $text; ?></span>
        <?php if ($hide_icon != true) { ?><img class="tooltip-out-of-stock-icon" title="Шины под заказ"
